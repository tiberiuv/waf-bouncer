...
async fn alert_webhook(
    ...
    let alert_webhook = alert_webhook
        .into_iter()
        .fold(IpRangeMixed::default(), |s, next| {
            s.merge(&IpRangeMixed::from(next))
        });
    ...
    Router::new()
        ...
        .layer(
            TraceLayer::new_for_http().make_span_with(move |request: &Request<_>| {
                ...
                match uri.path() {
                    "/" | "/api/v1/waf" => {
                        tracing::info_span!("proxy_request", uri = uri.to_string())
                    }
                    "/api/v1/health" => tracing::Span::none(),
                    _ => {
                        tracing::info_span!("http_request", uri = uri.to_string())
                    }
                }
            }),
        )
